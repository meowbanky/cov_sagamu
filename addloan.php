...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "eduEntry")) {
    ...
    $insertSQL = sprintf("INSERT INTO tbl_loan (memberid, periodid, loanamount) VALUES (%s, %s, %s)",
...
//        GetSQLValueString($cov,$_SESSION['period'], "int"),
//        GetSQLValueString($cov,$_POST['txtCoopid'], "text"),
//        GetSQLValueString($cov,$loanID, "int"),
//        GetSQLValueString($cov,doubleval($amount), "double"));
//
//
//    mysqli_select_db($cov,$database_cov);
...
//    $_POST['id'] = $_POST['PeriodId'];
    ...
    mysqli_select_db($cov,$database_cov);
    $query_name = "SELECT concat(ifnull(tbl_personalinfo.Lname,''),' ',ifnull(tbl_personalinfo.Fname,''),' ',ifnull(tbl_personalinfo.Mname,'')) as `name` FROM tbl_personalinfo WHERE memberid = '".$_POST['txtCoopid']."'";
    $name = mysqli_query($cov,$query_name) or die(mysqli_error($cov));
    $row_name = mysqli_fetch_assoc($name);
    $totalRows_name = mysqli_num_rows($name);

    $insertSQLBankSchedule = sprintf("INSERT INTO tbl_bank_schedule (memberid, name, periodid, loanamount) VALUES (%s, %s, %s, %s)",
        GetSQLValueString($cov,$_POST['txtCoopid'], "text"),
        GetSQLValueString($cov,$row_name['name'], "text"),
        GetSQLValueString($cov,$_SESSION['period'], "int"),
        GetSQLValueString($cov,doubleval($amount), "double"),
        GetSQLValueString($cov,$loanID, "int"));

    mysqli_select_db($cov,$database_cov);
    $Result1 = mysqli_query($cov,$insertSQLBankSchedule) or die(mysqli_error($cov));
...
mysqli_select_db($cov,$database_cov);
$query_Batch = sprintf("SELECT CONCAT(tbl_personalinfo.Lname,' , ',tbl_personalinfo.Fname,' ',(ifnull(tbl_personalinfo.Mname,' '))) AS `name`, (tbl_loan.loanamount) as loanamount, tbl_loan.loanid, tbl_loan.periodid, tbl_loan.memberid FROM tbl_personalinfo INNER JOIN tbl_loan ON tbl_loan.memberid = tbl_personalinfo.memberid WHERE tbl_loan.periodid= %s order by loanid desc", GetSQLValueString($cov,$col_Batch, "text"));
...
$importSettings = mysqli_query($cov,$query_importSettings) or die(mysqli_error($cov));
                                                            ...
                                                            <tr valign="top" align="left">
                                                                <td colspan="2" height="3"><div id="commodity2">
                                                                        <table width="96%" align="center" cellpadding="4" cellspacing="0">
                                                                            ...
                                                                            <tr valign="top" align="left">
                                                                                <td colspan="8" height="3"><img src="resource/spacer.gif" alt="" width="1" height="1" /><strong>Sum of Loan/Period = <?php if($totalRows_batchsum  > 0) {$row_batchsum['amount'] = 0; }echo number_format($row_batchsum['amount'] ,2,'.',','); ?></strong></td>
                                                                            </tr>
                                                                            </tbody>
                                                                        </table>
                                                                    </div></td>
