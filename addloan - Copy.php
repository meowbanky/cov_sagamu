...
//					   
//  mysql_select_db($database_cov, $cov);
//  $Result1 = mysql_query($insertSQL_MasterTransaction, $cov) or die(mysql_error());
...
$name = mysql_query($query_name, $cov) or die(mysql_error());
...
$totalRows_name = mysql_num_rows($name);
 ...
 $insertSQLBankSchedule = sprintf("INSERT INTO tbl_bank_schedule (memberid, name, periodid, loanamount) VALUES (%s, %s, %s, %s)",
                       GetSQLValueString($_POST['txtCoopid'], "text"),
					   GetSQLValueString(mysql_escape_string($row_name['name']), "text"),
					   GetSQLValueString($_SESSION['period'], "int"),
					   GetSQLValueString(doubleval($amount), "double"),
					   GetSQLValueString($loanID, "int"));

  mysql_select_db($database_cov, $cov);
  $Result1 = mysql_query($insertSQLBankSchedule, $cov) or die(mysql_error());
...
mysql_select_db($database_cov, $cov);
$query_batchsum = sprintf("SELECT (sum( tbl_loan.loanamount)) as amount FROM tbl_loan WHERE periodId =%s", GetSQLValueString($colname_batchsum, "int"));
$batchsum = mysql_query($query_batchsum, $cov) or die(mysql_error());
...
$importSettings = mysql_query($query_importSettings, $cov) or die(mysql_error());
$row_importSettings = mysql_fetch_assoc($importSettings);
$totalRows_importSettings = mysql_num_rows($importSettings);
             ...
             <tr>
               <td class="toplinks2" valign="top"><div align="justify">
                 <table class="Content" width="100%" border="0" cellpadding="4" cellspacing="0">
                   <tbody>
                     <tr>
                       <td valign="top"><form action="<?php echo $editFormAction; ?>" method="POST" name="eduEntry" onSubmit="return(validate()); ">
                         <fieldset>
                                   ...
                                   <table width="96%" align="center" cellpadding="4" cellspacing="0">
                                     <tbody>
                                       ...
                                       <tr valign="top">
                                         ...
                                         <td class="greyBgdHeader" valign="middle"><strong><div id="myBtn">Add Guarantor</div></strong></td>
                                       ...
                                       <?php do { ?>
                                           <?php if ($totalRows_Batch > 0) { // Show if recordset not empty ?>
                                             <tr valign="top">
                                               ...
                                               <td class="greyBgd" valign="middle"><div id="btn">Add Guarantor</div></td>
...
<div id="myModal" class="modal">
  ...
  <div class="modal-content">
    ...
    <p>Some text in the Modal..</p>
	  Name:</input>
	  <input name="surety" type="text" class="innerBox" id="CoopName1" onBlur="fill2();" onKeyUp="lookup2(this.value);" value="" size="30" / autocomplete="off">
	<div class="suggestionsBox" id="suggestions2" style="display: none;" >
                             <img src="upArrow.png" style="position: relative; top: -12px; left: 30px;" alt="upArrow" />
  </div>
<div class="suggestionList" id="autoSuggestionsList2" style="position: relative; top: 5px; "></div>
	Membership No:<input type="text" id="memberNo">
</div>
...
mysql_free_result($importSettings);

mysql_free_result($title);

mysql_free_result($logo);
?>
