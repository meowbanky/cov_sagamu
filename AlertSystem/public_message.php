...
WHERE patientid " . $equality ." ". $staffid . " AND Status = 'Active' GROUP BY
...
$query_payperiod = "SELECT tbpayrollperiods.Periodid, tbpayrollperiods.PayrollPeriod FROM tbpayrollperiods WHERE Periodid = ".$row_coopid2['Max(tlb_mastertransaction.periodid)'];
...
doSendMessage($recipients = $row_coopid2['MobilePhone'], $message = 'Your MHWUN ACCT. BALANCE, MONTHLY CONTR. : '.number_format($row_getContriubtion['contri_input'],2,'.',','). ' WELFARE SAVINGS: '.number_format($row_coopid2['Contribution'],2,'.',','). ' LOAN BAL: '.number_format($row_coopid2['Loanbalance'],2,'.',',').'  AS AT: '. $row_payperiod['PayrollPeriod']);
