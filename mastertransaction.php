...
// EXPORT PDF: the single #exportpdf handler (with the Download / Email / Both
// options) lives in getMasterTransaction.php, loaded with the export button.
// Removed the duplicate here so the two handlers no longer race.

// EXPORT EXCEL: the single #exportexcel handler (with the Download / Email /
// Both options) lives in getMasterTransaction.php, which is loaded together
// with the export button. Kept here previously as a duplicate — removed so the
// two handlers no longer race and suppress the options dialog.
