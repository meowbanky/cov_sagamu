...
$membersQuery = "SELECT memberid, CooperativeNo, CONCAT(Lname, ', ', Fname, ' ', IFNULL(Mname, '')) as full_name 
                 FROM tbl_personalinfo 
                 ORDER BY CooperativeNo";
    ...
    <div class="bg-white rounded-lg shadow-md p-6 mb-6">
        <form method="GET" class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
                ...
                <select name="memberid" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500" required>
                    <option value="">-- Select Member --</option>
                    <?php foreach ($members as $member): ?>
                        <option value="<?php echo $member['memberid']; ?>" <?php echo ($member['memberid'] == $selectedMember) ? 'selected' : ''; ?>>
                            <?php echo htmlspecialchars($member['CooperativeNo'] . ' - ' . $member['full_name']); ?>
                        </option>
                    <?php endforeach; ?>
                </select>
            </div>
            ...
            <div>
                ...
                <select name="from_period" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500">
                    <?php foreach (array_reverse($periods) as $period): ?>
                        <option value="<?php echo $period['Periodid']; ?>" <?php echo ($period['Periodid'] == $fromPeriod) ? 'selected' : ''; ?>>
            ...
            <div>
                ...
                <select name="to_period" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500">
                    <?php foreach ($periods as $period): ?>
                        <option value="<?php echo $period['Periodid']; ?>" <?php echo ($period['Periodid'] == $toPeriod) ? 'selected' : ''; ?>>
                            <?php echo htmlspecialchars($period['PayrollPeriod']); ?>
                        </option>
                    <?php endforeach; ?>
                </select>
            ...
            <div class="md:col-span-3">
                <button type="submit" class="w-full md:w-auto bg-blue-600 hover:bg-blue-700 text-white px-8 py-2 rounded-lg">
                ...
                <div>
                    ...
                    <div class="space-y-2 text-sm">
                        <div class="flex justify-between">
                            <span class="text-gray-600">Cooperative No:</span>
                            <span class="font-semibold"><?php echo htmlspecialchars($memberInfo['CooperativeNo']); ?></span>
                        </div>
                        <div class="flex justify-between">
                    ...
                    <div>
                        ...
                        <div class="space-y-2 text-sm">
                            <div class="flex justify-between p-2 bg-green-50 rounded">
                                ...
                                <span class="font-bold text-green-900">₦<?php echo number_format($summary['shares'], 2); ?></span>
                            </div>
                            <div class="flex justify-between p-2 bg-blue-50 rounded">
                                <span class="text-gray-700">Savings:</span>
                                <span class="font-bold text-blue-900">₦<?php echo number_format($summary['savings'], 2); ?></span>
                            </div>
                            <div class="flex justify-between p-2 bg-purple-50 rounded">
                                <span class="text-gray-700">Special Savings:</span>
                                <span class="font-bold text-purple-900">₦<?php echo number_format($summary['special_savings'], 2); ?></span>
                            </div>
                            ...
                            <div class="flex justify-between p-3 bg-gradient-to-r from-blue-100 to-green-100 rounded border-2 border-blue-500">
                                <span class="font-bold text-gray-900">Net Position:</span>
                                <span class="font-bold text-blue-900 text-lg">₦<?php echo number_format($summary['net_position'], 2); ?></span>
                            </div>
                        </div>
                    </div>
                <?php endif; ?>
                    ...
                    <table class="w-full">
                        <thead class="bg-gray-100">
                            <tr>
                                ...
                                <th class="px-4 py-3 text-right text-xs font-semibold text-gray-700 uppercase">Opening Balance</th>
                                ...
                                <th class="px-4 py-3 text-right text-xs font-semibold text-gray-700 uppercase">Closing Balance</th>
                            </tr>
                        </thead>
                        <tbody class="bg-white divide-y divide-gray-200">
                            <?php foreach ($transactions as $tx): ?>
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td class="px-4 py-3 text-sm text-right font-mono">₦<?php echo number_format($tx['opening_balance'], 2); ?></td>
                                    <td class="px-4 py-3 text-sm text-right font-mono <?php echo $tx['debit_amount'] > 0 ? 'text-red-600 font-semibold' : 'text-gray-400'; ?>">
                                        <?php echo $tx['debit_amount'] > 0 ? '₦' . number_format($tx['debit_amount'], 2) : '-'; ?>
                                    </td>
                                    <td class="px-4 py-3 text-sm text-right font-mono <?php echo $tx['credit_amount'] > 0 ? 'text-green-600 font-semibold' : 'text-gray-400'; ?>">
                                        <?php echo $tx['credit_amount'] > 0 ? '₦' . number_format($tx['credit_amount'], 2) : '-'; ?>
                                    </td>
                                    <td class="px-4 py-3 text-sm text-right font-mono font-bold">₦<?php echo number_format($tx['closing_balance'], 2); ?></td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
        ...
        <div class="bg-yellow-50 border-l-4 border-yellow-400 p-6 rounded-lg">
            <div class="flex items-center">
                ...
                <div>
                    ...
                    <p class="text-yellow-700 mt-1">Please select a member and period range to view their account statement.</p>
                </div>
            </div>
        </div>
    <?php endif; ?>
