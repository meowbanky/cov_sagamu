...
var frm_contacts;
var frm_message;
var dndmessage = "Notice!!! You have NOT selected a delivery method for MTN DND numbers! \n\nClick any one of the two options to either: \n\n1. Enable the DND option (2 units per MTN number). \n\n2. Disable DND delivery to MTN numbers (1 unit per MTN number).";
...
function countRecipients() {
    ...
    for (i = 0; i < frm_contacts.elements.length; i++) {
        if (frm_contacts.elements[i].type == 'checkbox' && frm_contacts.elements[i].id != 'SentMessageDndsender' && frm_contacts.elements[i].id != 'enable_schedule' && frm_contacts.elements[i].checked == true) {
...
function getContacts() {
    ...
    for (i = 0; i < frm_contacts.elements.length; i++) {
        if (frm_contacts.elements[i].type == 'checkbox' && frm_contacts.elements[i].checked == true && frm_contacts.elements[i].id != 'SentMessageDndsender' && frm_contacts.elements[i].id != 'enable_schedule') {
            listcontacts += parseInt(frm_contacts.elements[i].value);
            if (isNaN(listcontacts)) listcontacts = 0;
...
function make_unique(numbers) {
    ...
    numbers = arr_numbers.join(',');
    arr_numbers = numbers.match(regExp);
    if (arr_numbers == null) arr_numbers = Array();
    numbers = arr_numbers.join(',');
    num_recipients = arr_numbers.length;
    return numbers;
}
