...
function  GetSQLValueString($cov, $conn_vote, $theValue, $theType, $theDefinedValue = "", $theNotDefinedValue = "") 
    {
      $theValue = get_magic_quotes_gpc() ? stripslashes($theValue) : $theValue;

      $theValue = function_exists("mysqlii_real_escape_string") ? mysqlii_real_escape_string($conn_vote, $theValue) : mysqlii_escape_string($conn_vote, $theValue);
...
if ((isset($_POST["Submit"])) && ($_POST["Submit"] == "Save")) {
  $insertSQL = sprintf("INSERT INTO tbl_createcommodity (creationDate,items,costPrice,sellingPrice) VALUES (Now(),%s,%s,%s)",
                       	 GetSQLValueString($cov, $_POST['items'], "text"),
						 GetSQLValueString($cov, $_POST['costPrice'], "float"),
						 GetSQLValueString($cov, $_POST['SellingPrice'], "float"));
  mysqli_select_db($database_hms, $hms);
  $Result1 = mysqli_query($cov, $insertSQL, $hms) or die( mysqli_error($cov));
}
...
if ((isset($_POST["Submit"])) && ($_POST["Submit"] == "Update")) {
  $updateSQL = sprintf("UPDATE tbl_createcommodity SET items=%s,costPrice=%s,sellingPrice=%s WHERE itemsid=%s",
                      	 ...
                      	 GetSQLValueString($cov, $_POST['costPrice'], "float"),
						 GetSQLValueString($cov, $_POST['SellingPrice'], "float"),
					   	 GetSQLValueString($cov, $_POST['itemid'], "int"));

  mysqli_select_db($database_hms, $hms);
  $Result1 = mysqli_query($cov, $updateSQL, $hms) or die( mysqli_error($cov));
}
...
<title><?php echo $row_title['value']; ?> -  Dividend Calculator</title>
...
function getCalcDividend(){
	if(confirm("Are you sure you want to POST this Dividend Transaction?")){
				if (document.getElementById('period').value == "na"){
		alert("Please select Period");
			document.getElementById('period').focus();
		}else if (document.getElementById('percentage').value == ""){
		alert("Please Input Percentage");
			document.getElementById('percentage').focus();
		}else if(document.getElementById('dividendPeriod').value == "na"){
		alert("Please Select Posting Period");
		document.getElementById('dividendPeriod').focus();
		}else {
		
		var dividendPeriod = document.getElementById('dividendPeriod').value;
		var period = document.getElementById('period').value;
		var percentage = document.getElementById('percentage').value;
		//var periodid = document.getElementById('dividendPeriod').value
			
		var strURL="getCalcDividend.php?period="+period+"&percentage="+percentage+"&PeriodID="+dividendPeriod;
			...
			req.onreadystatechange = function() {
				if ((req.readyState == 4)) {
					...
					///if (req.status == 200) {						
						...
						alert("Dividend Posted Successfully");					
	...
	function getDeleteDividend(){
		if(confirm("Are you sure you want to DELETE this Dividend Transaction?")){
				if (document.getElementById('period').value == "na"){
		alert("Please select Period");
			document.getElementById('period').focus();
		}else if (document.getElementById('percentage').value == ""){
		...
		var strURL="getDeleteDividend.php?period="+period+"&percentage="+percentage+"&PeriodID="+dividendPeriod;
...
function getDividend() {		
		...
		var strURL="getDividend.php?period="+Period+"&percentage="+percentage;
		var req = getXMLHTTP();
  ...
  <tbody><tr>
    <td>Records Updated successful</td>
                        ...
                        <form action="<?php echo $editFormAction; ?>" method="POST" name="eduEntry" onSubmit="return(validate()); ">
                         <fieldset>
                         <legend class="contentHeader1"> Dividend<a name="top"></a></legend>
                         <table width="66%" align="center" cellpadding="4" cellspacing="0">
                           ...
                           <td colspan="6" height="1"><img src="skills_files/spacer.gif" width="1" height="1"></td>
                             ...
                             <tr valign="top" align="left">
                               <td height="27" colspan="6" align="center" class="greyBgd"> <strong class="tableHeaderContentDarkBlue">Dividend Calculator</strong></td>
                             </tr>
                             <tr valign="top" align="left">
                               <td height="35" align="right" valign="middle" class="greyBgd">Dividend Period</td>
                               <td colspan="5" align="left" valign="middle" class="greyBgd"><input name="dividend_period" type="text" class="innerBox" id="dividend_period"></td>
                             </tr>
                             <tr valign="top" align="left">
                               <td width="36%" height="35" align="right" valign="middle" class="greyBgd"><p>Percentage:</p></td>
                               <td colspan="5" align="left" valign="middle" class="greyBgd"><input name="percentage" type="text" class="innerBox" id="percentage" onKeyUp="return checknum(this)"></td>
                             </tr>
                             <tr align="center" valign="top" class="greyBgd">
                               <td height="10" align="right" valign="middle" class="greyBgd">End of Period:</td>
                               
                               <td width="12%" align="left" valign="middle"><select name="period" id="period">
...
?>
                             ...
                             <tr valign="top" align="center">
                               <td colspan="6" valign="middle" height="10"><input name="Search" type="button" class="formbutton" id="Submit" value="Search" onClick="getDividend()"></td>
                             </tr>
                             <tr valign="top" align="center">
