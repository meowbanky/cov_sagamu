...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "eduEntry")) {
	$amount = str_replace(",","",$_POST['txtCommodityPrice']);
	$monthlyRepayment = str_replace(",","",$_POST['txtMonthlyRepayment']);
	
  $insertSQL = sprintf("INSERT INTO tbl_commodity (itemsid, amount, monthlyRepayment, periodId,memberId) VALUES (%s, %s, %s, %s,%s)",
                       GetSQLValueString($_POST['Itemid'], "int"),
                       GetSQLValueString($amount, "double"),
                       GetSQLValueString($monthlyRepayment, "double"),
  ...
  $insertSQL_MasterTransaction = sprintf("INSERT INTO tlb_mastertransaction (periodid, memberid, itemsid, commodityAmount,commodityID) VALUES (%s, %s, %s, %s,%s)",
                      GetSQLValueString($_POST['PeriodId'], "int"),
					  GetSQLValueString($_POST['txtCoopid'], "text"),
					   GetSQLValueString($_POST['Itemid'], "int"),
                       GetSQLValueString($amount, "double"),
                       GetSQLValueString($commodityID, "double"));
					   
  mysql_select_db($database_hms, $hms);
  $Result1 = mysql_query($insertSQL_MasterTransaction, $hms) or die(mysql_error());
  $_POST['id'] = $_POST['PeriodId'];
  
  $update_contributions = sprintf("UPDATE tbl_contributions SET commodity = %s where membersid = %s",
                      GetSQLValueString($monthlyRepayment, "double"),
                       GetSQLValueString($_POST['txtCoopid'], "text"));
					   
  mysql_select_db($database_hms, $hms);
  $Result34 = mysql_query($update_contributions, $hms) or die(mysql_error());
...
mysql_select_db($database_hms, $hms);
$query_commodity = "SELECT tbl_createcommodity.itemsid, tbl_createcommodity.items FROM tbl_createcommodity";
$commodity = mysql_query($query_commodity, $hms) or die(mysql_error());
...
<title>MHWUN, OOUTH -  Add Commodity Item</title>
...
function getCommodityBalance(id) {		
		
		var strURL="commodityBalance.php?id="+id;
		var req = getXMLHTTP();
		...
		if (req) {
			...
			req.onreadystatechange = function() {
				if (req.readyState == 4) {
					// only if "OK"
					if (req.status == 200) {						
						document.getElementById('txtcommodityBalance').innerHTML=req.responseText;						
					} else {
...
function deleteCommodityItem(commodityID,periodid) {		
		
		
		if (confirm("Are you sure your want to delete selected item(s)")){
		
	var ln = 0;
	var checkbox = document.getElementsByName('commodityID');
	...
	for (i=0;i<checkbox.length;i++){
		...
		var strURL="deleteCommodity.php?commodityID="+coopid;
		var req = getXMLHTTP();
	...
	function getCommodity(id) {		
		...
		var strURL="batchcommodity.php?id="+id;
		var req = getXMLHTTP();
...
document.forms[0].txtCommodityBalance.value = ""
fo();
}
</script>
<script language="JavaScript" type="text/JavaScript">
  ...
  <tr>
   <td class="innerPg" valign="top"><table width="900" border="0" cellpadding="0" cellspacing="0">
     ...
     <tr>
       ...
       <td rowspan="2" class="Content" valign="top"><img src="education_files/mycv.gif" width="350" height="30">
             ...
             <tr>
               <td class="toplinks2" valign="top"><div align="justify">
                 <table class="Content" width="100%" border="0" cellpadding="4" cellspacing="0">
                   <tbody>
                     <tr>
                       <td valign="top"><form action="<?php echo $editFormAction; ?>" method="POST" name="eduEntry" onSubmit="MM_validateForm('txtBankName','','R','txtBankAccountNo','','R','txtbankcode','','R','txtAmount','','R','txNarration','','R');return document.MM_returnValue">
                         <fieldset>
                           ...
                           <table width="97%" align="center" cellpadding="4" cellspacing="0">
                             <tbody>
                               ...
                               <tr valign="top" align="left">
                                 ...
                                 <td class="greyBgd" valign="middle" align="left"><input name="CoopName" type="text" class="innerBox" id="CoopName" onBlur="fill();" onKeyUp="lookup(this.value);" value="" size="30" / autocomplete="off">
                                   <input type="button" class="formbutton" onClick="javascript:clearBox()" value="X">
                                   <br></td>
                                 <td width="69%" rowspan="10" align="left" valign="top" class="greyBgd"><div id="hide">
                                   <iframe id="opatdetails" frameborder="0" src="" width=0 height=0 style="overflow-style:auto"></iframe>
                                   </div>
                                   <br>
                                   <div id="hide2">
                                     <iframe id="editAmount" frameborder="0" src="" width=0 height=0 style="overflow-style:auto"></iframe>
                                     </div></td>
                               ...
                               <tr valign="top" align="left">
                                 ...
                                 <td class="greyBgd" valign="middle" align="left"><input name="txtCoopid" type="text" class="innerBox" id="txtCoopid" readonly onMouseOver="getCommodityBalance(this.value)"></td>
                                 </tr>
                               <tr valign="top" align="left">
                                 <td class="greyBgd" valign="middle" align="right" height="35">Commodity Balance:</td>
                                 <td class="greyBgd" valign="middle" align="left"><div id="txtcommodityBalance"><input name="txtCommodityBalance" type="text" class="innerBox" id="txtCommodityBalance" value="0.00" readonly></div></td>
                               </tr>
                               <tr valign="top" align="left">
                                 <td class="greyBgd" valign="middle" width="31%" align="right" height="35">Comodity Item:</td>
                                 <td class="greyBgd" valign="middle" width="69%" align="left"><select name="Itemid" id="Itemid" onChange="Javascript:getCommodityPrice(this.value)">
                                   <option value="na">Select</option>
                                   <?php
do {  
?>
                                   <option value="<?php echo $row_commodity['itemsid']?>"><?php echo $row_commodity['items']?></option>
                                   <?php
} while ($row_commodity = mysql_fetch_assoc($commodity));
  $rows = mysql_num_rows($commodity);
  if($rows > 0) {
...
?>
                                   ...
                                   <input name="button2" type="button" class="formbutton" id="button2" value="Add Commodity" onClick='location.href="create_commodity_item.php"'></td>
                                 </tr>
                               <tr valign="top" align="left">
                                 <td class="greyBgd" valign="middle" width="31%" align="right" height="28">Price.  :</td>
                                 <td class="greyBgd" valign="middle" width="69%" align="left"><label></label>
                                   <div id="commodityPrice">
                                     <input name="txtCommodityPrice" type="text" class="innerBox" id="txtCommodityPrice" onChange="this.value=number_format(this.value);" size="60" readonly>
                                     *</div></td>
                                 </tr>
                               <tr valign="top" align="left">
                                 <td class="greyBgd" valign="middle" width="31%" align="right" height="28">Monthly Repayment:</td>
                                 <td class="greyBgd" valign="middle" width="69%" align="left"><div id="bankcode">
                                   <input name="txtMonthlyRepayment" type="text" class="innerBox" id="txtMonthlyRepayment" size="60" / onKeyUp="this.value=number_format(this.value);">
                                   * </div></td>
                                 </tr>
                               <tr valign="top" align="left">
...
?>
                               ...
                               <tr valign="top" align="left">
                                 <td colspan="3" valign="middle" align="center" height="10"><input name="Submit2" type="submit" class="formbutton" value="Add Commodity" onClick="submith()">
...
document.eduEntry.CoopName.focus();
                         ...
                         <br>
                         <fieldset>
                           <legend class="contentHeader1">Commodity Preview </legend>
                           <script language="JavaScript" type="text/JavaScript">
<!--
function GP_popupConfirmMsg(msg) { //v1.0
  document.MM_returnValue = confirm(msg);
                           ...
                           <table width="96%" align="center" cellpadding="4" cellspacing="0">
                             <tbody>
                               ...
                               <tr valign="top" align="right">
                                 <td class="content" align="left" height="1"><a href="addcommodity.php?id=">Add</a></td>
                                 <td class="content" height="1"><a >
                                   <form name="period2">
                                     <select name="PeriodId2" id="PeriodId2" onChange="getCommodity(document.forms['period2'].PeriodId2.value)">
                                       <option value="na">Select Period</option>
                                       <?php
do {  
?>
