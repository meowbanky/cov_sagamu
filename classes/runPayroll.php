...
// echo '<div id="information" style="width" ><p align="center"></p> </div> ';
		...
		if ($existtrans) { 
			...
			mysqli_select_db($connect, 'salary');
			$query_masterTransaction = 'SELECT * FROM employee WHERE STATUSCD = "A"';
			$masterTransaction = mysqli_query($connect,$query_masterTransaction) or die(mysql_error());
        ...
        try{ //echo $period ;
             ...
             {	//$percent = '';
             		...
             		deletecurrentstaffPayslip($row['staff_id'],$period);
             		
             		$queryMaster = $conn->prepare('INSERT INTO master_staff (staff_id,PPNO,NAME,DEPTCD,BCODE,ACCTNO,GRADE,STEP,period,PFACODE,PFAACCTNO) VALUES (?,?,?,?,?,?,?,?,?,?,?)');
              	$master = $queryMaster->execute(array($row['staff_id'],$row['PPNO'],$row['NAME'],$row['DEPTCD'],$row['BCODE'],$row['ACCTNO'],$row['GRADE'],$row['STEP'],$period,$row['PFACODE'],$row['PFAACCTNO']));
              	...
              	while ($row_allow = array_shift($out_allow)) 
              	{
              		...
              		$query_value = $conn->prepare('SELECT allowancetable.`value` FROM allowancetable WHERE allowancetable.grade = ? AND allowancetable.step = ? AND allowcode = ?');
			  				...
			  				}else if(intval($row_deduct['edType']) == '4')
			  				{
			  					$sql_loancheck = "SELECT tbl_earning_deduction_type.edType FROM tbl_earning_deduction_type INNER JOIN tbl_earning_deduction ON tbl_earning_deduction.edType = tbl_earning_deduction_type.edType WHERE tbl_earning_deduction.ed_id = '".$row_deduct['allow_id'] ."' and tbl_earning_deduction_type.edType = 4" ;
									...
									if($total_loancheck > 0)
									{
										...
										$sql_repayment = "SELECT tbl_repayment.staff_id, tbl_repayment.allow_id, (SUM(ifnull(tbl_repayment.value,0)) + SUM(ifnull(tbl_repayment.cashPay,0))) as repayment FROM tbl_repayment WHERE staff_id = '".$row['staff_id']."' and allow_id = '".$row_deduct['allow_id']."'";
										...
										$balance = $row_loan['loan'] - $row_repayment['repayment'];
											...
											if(floatval($balance) > floatval($row_deduct['value']))
											{
												...
												try{
													...
													$conn->prepare($query_repayment)->execute(array($row['staff_id'], $row_deduct['allow_id'], $output, $period,$_SESSION['SESS_MEMBER_ID'],$recordtime));
			  			...
			  			if(lastPayCheck($row['staff_id'],$period) == '1')
								...
								$queryUdateStatus = 'UPDATE employee SET STATUSCD = ? WHERE staff_id = ? ';
								$conn->prepare($queryUdateStatus)->execute(array('R',$row['staff_id']));
										
								}
											
									// dedduct development levy		
								if($_SESSION['activeYear'] > 2020){
//									if(devlevyCheck($row['staff_id'],$_SESSION['activeYear']) == '1')	
//									{
//										
//									}else{
//										
//									//$recordtime = date('Y-m-d H:i:s');
//									$query = 'INSERT INTO tbl_devlevy (staff_id,period_year,period) VALUES (?,?,?)';
//									$conn->prepare($query)->execute(array($row['staff_id'],$_SESSION['activeYear'],$period));
//										
//										
//									$recordtime = date('Y-m-d H:i:s');
//									$query = 'INSERT INTO tbl_master (staff_id, allow_id, deduc, type, period,editTime,userID) VALUES (?,?,?,?,?,?,?)';
//									$conn->prepare($query)->execute(array($row['staff_id'], '79', '100', '2',  $period,$recordtime,$_SESSION['SESS_MEMBER_ID']));
//									}	
								}	
 ...
 // begin of payrol run if it had been ran earlier   	
    	mysqli_select_db($connect, 'salary');
			$query_masterTransaction = "SELECT * FROM employee INNER JOIN tbl_periodicafterrun ON tbl_periodicafterrun.staff_id = employee.staff_id WHERE tbl_periodicafterrun.period = '{$period}'";
			$masterTransaction = mysqli_query($connect,$query_masterTransaction) or die(mysql_error());
			$row_masterTransaction = mysqli_fetch_assoc($masterTransaction);
			$totalRows_masterTransaction = mysqli_num_rows($masterTransaction);
 	 		$total = $totalRows_masterTransaction ;
        ...
        try{ //echo $period ;
              ...
              global $conn;
              $query = $conn->prepare('SELECT * FROM employee INNER JOIN tbl_periodicafterrun ON tbl_periodicafterrun.staff_id = employee.staff_id WHERE tbl_periodicafterrun.period = ?');
             ...
             {	
             	...
             	try{
						       	...
						       	$query = 'DELETE FROM tbl_devlevy where period = ? and staff_id = ?';
										$conn->prepare($query)->execute(array($period,$row['staff_id']));
						         ...
						         $payrollquery2->execute(array($period,$row['staff_id']));
						         $deduc = $payrollquery2->fetchAll(PDO::FETCH_ASSOC);
						        
						         foreach ($deduc as $row23 => $link2) 
						         {
...
//									}	
        ...
        echo $e->getMessage();
        			...
        			$statuschange = $conn->prepare('DELETE FROM tbl_periodicafterrun WHERE period = ?');
        			$perres = $statuschange->execute(array($period));
