...
ini_set('max_execution_time','0');
 ...
 $percent;
					  	...
					  	$res = $query->execute(array($step, $grade, $staff_id));
							...
							$query_masterTransaction = "SELECT * FROM employee WHERE staff_id = '{$_POST['curremployee']}'";
									...
									}else
									{
									 ...
									 // echo $row_allow['allow_id'].' '.$row_allow['edDesc'].' '.number_format($output).'<br>';
									  try{
										...
										$queryUdate = 'UPDATE allow_deduc SET value = ?,counter = 0 WHERE allow_id = ? AND staff_id = ? ';
