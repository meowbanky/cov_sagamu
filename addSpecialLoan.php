...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "eduEntry")) {
  ...
  $insertSQL = sprintf("INSERT INTO tbl_special_loan (memberid, periodid, loanamount) VALUES (%s, %s, %s)",
                       GetSQLValueString($cov,$_POST['txtCoopid'], "text"),
					   GetSQLValueString($cov,$_SESSION['period'], "int"),
					   GetSQLValueString($cov,doubleval($amount), "double"));
					   //GetSQLValueString($cov,$_POST['txtInterest'], "double"));

  mysqli_select_db($cov,$database_cov);
  $Result1 = mysqli_query($cov,$insertSQL) or die(mysql_error());
    
  $loanID = mysqli_insert_id($cov);
...
mysqli_select_db($cov,$database_cov);
$query_Batch = sprintf("SELECT CONCAT(tbl_personalinfo.Lname,' , ',tbl_personalinfo.Fname,' ',(ifnull(tbl_personalinfo.Mname,' '))) AS `name`, 
(tbl_special_loan.loanamount) as loanamount, tbl_special_loan.loanid, tbl_special_loan.periodid, tbl_special_loan.memberid 
FROM tbl_personalinfo INNER JOIN tbl_special_loan ON tbl_special_loan.memberid = tbl_personalinfo.memberid WHERE tbl_special_loan.periodid = %s ", GetSQLValueString($cov,$col_Batch, "text"));
$Batch = mysqli_query($cov,$query_Batch) or die(mysql_error());
$row_Batch = mysqli_fetch_assoc($Batch);
$totalRows_Batch = mysqli_num_rows($Batch);

$colname_batchsum = "-1";
if (isset($_SESSION['period'])) {
  $colname_batchsum = $_SESSION['period'];
}
mysqli_select_db($cov,$database_cov);
...
$batchsum = mysqli_query($cov,$query_batchsum) or die(mysql_error());
$row_batchsum = mysqli_fetch_assoc($batchsum);
$totalRows_batchsum = mysqli_num_rows($batchsum);

mysqli_select_db($cov,$database_cov);
$query_importSettings = "SELECT * FROM tbl_settings";
$importSettings = mysqli_query($cov,$query_importSettings) or die(mysql_error());
$row_importSettings = mysqli_fetch_assoc($importSettings);
$totalRows_importSettings = mysqli_num_rows($importSettings);
...
//$check_Period = mysql_query($query_check_Period, $cov) or die(mysql_error());
//$row_check_Period = mysql_fetch_assoc($check_Period);
//$totalRows_check_Period = mysql_num_rows($check_Period);
...
<title><?php echo $row_title['value']; ?> -  Add Loan</title>
...
function getXMLHTTP() { //fuction to return the xml http object
    ...
    function submitt(){
        ...
        $('#eduEntry').submit(function(e){ 
                e.preventDefault();
             
       if( document.getElementById('sessionPeriod').value == "" )
   {
     alert( "Please Select Period!" );
     document.eduEntry.PeriodId.focus() ;
     return false;
   } 
       if( document.eduEntry.txtCoopid.value.trim() == "" )
   {
     alert( "Please provide Member's No!" );
     document.eduEntry.CoopName.focus() ;
   return false;
  }
            if( document.eduEntry.txtAmountGranted.value.trim() == "" )
   {
     alert( "Please provide Amount Granted!" );
     document.eduEntry.txtAmountGranted.focus() ;
     return false;
   }
        document.getElementById('wait').style.visibility = "visible";
        document.getElementById('Submit2').setAttribute("disabled","disabled");
        document.getElementById('Submit2').setAttribute("Value","Saving..");
        
        document.forms[0].submit();
        //$('#eduEntry').submit();
        document.getElementById('wait').style.visibility = "hidden";
        document.getElementById('Submit2').setAttribute("disabled","enable");
        document.getElementById('Submit2').setAttribute("text","Add Loan");
...
function deleteLoan(commodityID,periodid) {		
	...
	for (i=0;i<checkbox.length;i++){
		...
		var strURL="deleteSpecialLoan.php?loanID="+loanID;
	...
	window.location = 'addSpecialLoan.php';
	}
	...
	function getLoan(id) {		
		...
		var strURL="batchSpecialLoan.php?id="+id;
		var req = getXMLHTTP();
...
<!-- fwtable fwsrc="Untitled" fwbase="top.gif" fwstyle="Dreamweaver" fwdocid = "2000728079" fwnested="0" -->
  ...
  <tr>
   <td colspan="4" rowspan="4" align="center"><img name="top_r2_c1" src="resource/spacer.gif" alt="" width="1" border="0" height="1"><img src="<?php echo $row_logo['value']; ?>"><img name="top_r4_c4" src="resource/spacer.gif" alt="" width="1" border="0" height="1"></td>
    <td>&nbsp;</td>
           ...
           <tbody>
             <tr>
               <td class="sidenavtxt" align=""><em><font size="1" face="Verdana, Arial, Helvetica, sans-serif">Welcome,</font></em> <font size="1" face="Verdana, Arial, Helvetica, sans-serif"><span><?php echo ($_SESSION['FirstName']); ?><br>
                 <img src="resource/spacer.gif" alt="" width="1" height="8" border="0"><img src="resource/arrow_bullets2.gif" alt="" border="0"> <a href="index.php">Logout</a> </span></font></td>
             </tr>
           </tbody>
         </table>
         <br>
  ...
  <tbody><tr>
    ...
    <tr>
      ...
      <td class="sidenavtxt" align=""><a href="addloan.php">Add Loan</a></td>
       ...
       <td rowspan="2" class="Content" valign="top"><img src="resource/addloan.gif" width="350" height="30">
             ...
             <tr>
               <td class="toplinks2" valign="top"><div align="justify">
                 <table class="Content" width="100%" border="0" cellpadding="4" cellspacing="0">
                   <tbody>
                     <tr>
                       <td valign="top"><form action="<?php echo $editFormAction; ?>" method="POST" name="eduEntry" id="eduEntry" >
                         <fieldset>
                           <legend class="contentHeader1">Add Special Loan</legend>
                           <table width="97%" align="center" cellpadding="4" cellspacing="0">
                             <tbody>
                               <tr valign="top" align="left">
                                 <td colspan="2" height="46"><img src="resource/spacer.gif" width="1" height="1"></td>
                                 </tr>
                               <tr valign="top" align="left">
                                 <td width="15%" height="35" align="right" valign="middle" class="greyBgd">Name</td>
                                 <td width="39%" align="left" valign="middle" class="greyBgd"><input name="CoopName" type="text" class="innerBox" id="CoopName" onBlur="fill();" onKeyUp="lookup(this.value);" value="" size="30" / autocomplete="off">
                             ...
                             <div class="suggestionList" id="autoSuggestionsList" style="position: relative; top: 5px; "></div>
                               ...
                               <tr valign="top" align="left">
                                 <td class="greyBgd" valign="middle" align="right" height="35"><p>Member No:</p></td>
                                 <td class="greyBgd" valign="middle" align="left"><input name="txtCoopid" type="text" class="innerBox" id="txtCoopid" readonly onMouseOver="getLoanBalance(this.value)"></td>
                                 </tr>
                               <tr valign="top" align="left">
                                 <td class="greyBgd" valign="middle" align="right" height="35"><p>Loan Balance:</p>
                                   <p>Interest Balance:</p></td>
                                 <td class="greyBgd" valign="middle" align="left"><div id="loanBalance"><input name="txtLoanBalance" type="text" class="innerBox" id="txtLoanBalance" readonly><br><input name="interestbalance" type="text" class="innerBox" id="interestbalance" autocomplete="off" readonly></div>
                                   </td>
                               </tr>
                               <tr valign="top" align="left">
                                 <td class="greyBgd" valign="middle" align="right" height="35">Amount Granted</td>
                                 <td class="greyBgd" valign="middle" align="left"><input name="txtAmountGranted" type="text" class="innerBox" id="txtAmountGranted" onKeyUp="return checknum(this)"  autocomplete="off"  onChange="this.value=number_format(this.value)"></td>
                               </tr>
                               <tr valign="top" align="left">
                                 <td class="greyBgd" valign="middle" align="right" height="35">Period</td>
                                 <td class="greyBgd" valign="middle" align="left"><div id="div">
                                   <select name="PeriodId" id="PeriodId" onChange="setvariablechat()" >
...
?>
                                   ...
                                   <input name="Submit3" class="formbutton" value="Set Period" type="button" onClick="location.reload(true);">
                                   <input name="sessionPeriod" type="hidden" id="sessionPeriod" value="<?php if(isset($_SESSION['period'])){ echo $_SESSION['period'];} ?>"></div></td>
                               </tr>
                               <tr valign="top" align="left">
                                 <td class="greyBgd" valign="middle" align="right" height="35"><div id="wait"  style="background-color:white;visibility:hidden;border: 1px solid black;padding:5px;" class="overlay">
 <img src="images/pageloading.gif" class="area">Please wait...
 </div></td>
                                 <td class="greyBgd" valign="middle" align="left">&nbsp;</td>
                                 </tr>
                               <tr valign="top" align="left">
                                 <td colspan="3" valign="middle" align="center" height="10"><input name="Submit2" id="Submit2" type="submit" class="formbutton" value="Add Loan" onClick="submitt()">
                         ...
                         <input name="interestRate" type="hidden" id="interestRate" value="<?php echo $row_importSettings['interestRate']; ?>">
                       </form>
                         <script language="JavaScript" type="text/JavaScript">
document.eduEntry.CoopName.focus();
                        </script>
                         <br>
                         <fieldset>
                           <legend class="contentHeader1">Loan Preview </legend>
                           <script language="JavaScript" type="text/JavaScript">
<!--
function GP_popupConfirmMsg(msg) { //v1.0
  document.MM_returnValue = confirm(msg);
...
//-->
                           ...
                           <table width="96%" align="center" cellpadding="4" cellspacing="0">
                             <tbody>
                               <script language="javascript">document.getElementById("PeriodId2").selectedIndex</script>
                               <tr valign="top" align="right">
                                 <td class="content" align="left" height="1"></td>
                                 <td class="content" height="1"><a >
                                   <form name="period2">
                                     <select name="PeriodId2" id="PeriodId2" onChange="getLoan(document.forms['period2'].PeriodId2.value)">
                                       <option value="na">Select Period</option>
                                       <?php
do {  
?>
...
?>
                               ...
                               <tr valign="top" align="left">
                                 <td colspan="2" height="3"><div id="commodity"></div></td>
                                 </tr>
                               <tr valign="top" align="left">
                                 <td colspan="2" height="3"><div id="commodity2">
                                   <table width="96%" align="center" cellpadding="4" cellspacing="0">
                                     <tbody>
                                       ...
                                       <?php do { ?>
                                           <?php if ($totalRows_Batch > 0) { // Show if recordset not empty ?>
                                             <tr valign="top">
                                               ...
                                               <td class="greyBgd" valign="middle">Add Guarantor</td>
...
<?php } while ($row_Batch = mysqli_fetch_assoc($Batch)); ?>
                                       <tr valign="top" align="left">
                                         <td colspan="8" height="3"><img src="resource/spacer.gif" alt="" width="1" height="1" /><strong>Sum of Loan/Period = <?php echo number_format($row_batchsum['amount'] ,2,'.',','); ?></strong></td>
                                         </tr>
                                       </tbody>
                                     </table>
                                   </div></td>
...
mysqli_free_result($importSettings);
