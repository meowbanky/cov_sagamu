...
$query_loan = sprintf("SELECT tbl_special_loan.memberid FROM tbl_special_loan WHERE tbl_special_loan.loanid = %s ", GetSQLValueString($cov,$col_loanID, "int"));
...
if ((isset($_GET['loanID'])) && ($_GET['loanID'] != "")) {
  $deleteSQL = sprintf("DELETE FROM tbl_special_loan WHERE loanid=%s",
                       GetSQLValueString($cov,$_GET['loanID'], "int"));
$deleteSQL_masterTransact = sprintf("DELETE FROM tlb_mastertransactionSpecial WHERE loanid=%s",
                       GetSQLValueString($cov,$_GET['loanID'], "int"));		
...
$deleteShedule = sprintf("DELETE FROM tbl_bank_schedule WHERE loanid=%s",
  ...
  $Result2 = mysqli_query($cov,$deleteSQL_masterTransact) or die(mysql_error());
  $Result3 = mysqli_query($cov,$deleteShedule) or die(mysql_error());
    
	$deleteGoTo = 'addloanSpecialLoan.php';
