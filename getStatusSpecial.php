...
//$query_status = sprintf("SELECT tbl_personalinfo.patientid, concat(tbl_personalinfo.Lname,' , ', tbl_personalinfo.Fname,' ', ifnull( tbl_personalinfo.Mname,'')) as namess, (sum(tlb_mastertransactionspecial.Contribution)+sum(tlb_mastertransactionspecial.withdrawal)) as Contribution, (sum(tlb_mastertransactionspecial.loanAmount)+ sum(tlb_mastertransactionspecial.interest)) as Loan, ((sum(tlb_mastertransactionspecial.loanAmount)+ sum(tlb_mastertransactionspecial.interest))- (sum(tlb_mastertransactionspecial.loanRepayment)+ifnull(sum(tlb_mastertransactionspecial.repayment_bank),0))) as Loanbalance, sum(tlb_mastertransactionspecial.withdrawal) as withdrawal FROM tlb_mastertransactionspecial INNER JOIN tbl_personalinfo ON tbl_personalinfo.patientid = tlb_mastertransactionspecial.memberid where patientid = %s AND tlb_mastertransactionspecial.periodid <= %s GROUP BY patientid", GetSQLValueString($col_status, "text"),GetSQLValueString($col_period, "int"));
...
FROM
tbl_personalinfo
LEFT JOIN tlb_mastertransactionspecial ON tbl_personalinfo.memberid = tlb_mastertransactionspecial.memberid
...
where tbl_personalinfo.memberid = %s AND tlb_mastertransactionspecial.periodid <= %s GROUP BY memberid", GetSQLValueString($cov,$col_status, "text"),GetSQLValueString($cov,$col_period, "int"));
