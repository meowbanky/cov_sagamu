...
function process2(){
	...
	if (document.getElementById('PeriodId2').value == "na"){
		alert("Please select a Period to Process Transaction")} else{
	 var periodid = document.getElementById('PeriodId2').value;
	 window.location.href="processSPLoan.php?PeriodID="+periodid;
	}
}
function process(){
	//alert(document.getElementById('PeriodId').value);
	if (document.forms['eduEntry'].PeriodId.value == "na"){
		alert("Please select a Period to Process Transaction")} else{
	 var periodid = document.forms['eduEntry'].PeriodId.value;
	 window.location.href="processSPLoan.php?PeriodID="+periodid;
	}
}
function validate(){
  ...
  <tbody><tr>
    <td>Records Updated successful</td>
                        ...
                        <form action="<?php echo $editFormAction; ?>" method="POST" name="eduEntry" onSubmit="return(validate()); ">
                         <fieldset>
                         <legend class="contentHeader1"> Process Special Loan<a name="top"></a></legend>
