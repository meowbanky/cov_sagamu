    ...
    <form id="notificationForm" action="save_notification.php" method="POST">
        ...
        <div class="mb-4">
            ...
            >
                ...
                $query = "SELECT tblusers.UserID, tbl_personalinfo.onesignal_id, concat(tbl_personalinfo.Lname,' ',tbl_personalinfo.Fname) as name FROM tblusers
                          INNER JOIN tbl_personalinfo ON tblusers.UserID = tbl_personalinfo.memberid
                          WHERE (tbl_personalinfo.onesignal_id) != ''";
                $stmt = $db->query($query);
                ...
                while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                    ...
                    $name = $row['name'];
                    echo "<option value='{$memberid},{$deviceId}'>{$name} (Coop No: {$memberid})</option>";
                }
                ?>
            </select>
        </div>
