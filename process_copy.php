...
mysqli_select_db($cov, $database_cov);
$query_entry = "SELECT tbl_globa_settings.`value` FROM tbl_globa_settings where setting_id = 7";
$entrySettings  = mysqli_query($cov, $query_entry) or die(mysqli_error($cov));
$row_entry  = mysqli_fetch_assoc($entrySettings);
$totalRows_entry  = mysqli_num_rows($entrySettings);

$entryFees = (int)$row_entry[`value`];
...
WHERE tbl_personalinfo.memberid = %s GROUP BY memberid", GetSQLValueString($cov, $row_member['memberid'], "text"));
			...
			mysqli_select_db($cov, $database_cov);
			$query_deductions = "SELECT IFNULL(sum(tbl_contributions.contribution),0) as contri FROM tbl_contributions WHERE membersid = '" . $row_member['memberid'] . "' AND periodid = '" . $_GET['PeriodID'] . "' AND pay_method = 0 GROUP BY membersid";
			...
			//echo $row_member['memberid'] .' - '.$row_deductions['contri'] ;
