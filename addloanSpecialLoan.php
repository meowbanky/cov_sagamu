...
<div class="container mt-6">
    <div class="flex justify-between items-center mb-4">
        <h2 class="text-2xl font-semibold mb-4">Add / Edit Special Loan (<?= $sp_rate_display ?>% Interest)</h2>
    ...
    <form id="loanForm" autocomplete="off" class="space-y-4 bg-white p-4 rounded shadow">
        ...
        <p class="text-sm text-gray-500 mt-1">Interest of <?= $sp_rate_display ?>% will be automatically calculated.</p>
        ...
        <div class="flex space-x-2 mt-2">
            <button type="submit" id="submitBtn" class="bg-blue-600 text-white px-4 py-2 rounded shadow">Add Special Loan</button>
...
$(function() {
    ...
    $("#PeriodId").on('change', function() {
        ...
        window.history.replaceState({}, '', 'addloanSpecialLoan.php?period=' + pid);
    });
    ...
    $("#loanForm").on("submit", function(e) {
        ...
        $.post("save_special_loan.php", formData, function(resp) {
            // Always expect JSON!
            if (resp.success) {
    ...
    $(document).on("click", ".edit-loan-btn", function(e) {
        ...
        $.get("get_special_loans_by_period.php", {
            loanid: loanid,
            action: 'get_one'
        }, function(row) {
            ...
            $("#submitBtn").text("Update Special Loan");
            $("#cancelEditBtn").show();
        }, 'json');
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.post("deleteSpecialLoan.php?loanID=" + loanid, {}, function(resp) { 
                    // deleteSpecialLoan.php redirects, so we might need to handle it differently or update it to JSON.
                    // For now assuming it works as legacy. Ideally refactor deleteSpecialLoan to return JSON.
                    // But waiting for result...
                    updateLoansTable(pid);
                    resetForm();
    ...
    function updateLoansTable(pid) {
        ...
        $.post("get_special_loans_by_period.php", {
            periodid: pid,
            action: 'list'
        }, function(data) {
