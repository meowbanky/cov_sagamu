<?php
	...
	function retrieveEmployees($table, $filter1, $filter2, $basevar, $sortvar){
		...
		function retrieveCountry($default='Nigeria'){
		global $conn;
		...
		try{
			...
			while ($row = array_shift($out)) {
				echo'<option ';
				 if($row['Country'] == $default){echo "Selected" ;}
				 echo ' value="' . $row['Country'] .'">' . $row['Country']  . '</option>';
			}
		}
		catch(PDOException $e){
			echo $e->getMessage();
	...
	function retrieveLg($state_id,$selectedLg=-1){
		global $conn;

		try{
			$query = $conn->prepare('SELECT * FROM lg where State = ?');
			$res = $query->execute(array($state_id));
			$out = $query->fetchAll(PDO::FETCH_ASSOC);
			
			while ($row = array_shift($out)) {
				echo'<option ';
				if($row['Local_Government'] == $selectedLg){echo "Selected" ;}
				echo ' value="'.  $row['Local_Government'] .'">' . $row['Local_Government']  . '</option>';
			}
		}
		catch(PDOException $e){
			echo $e->getMessage();
	...
	function retrieveState($country_id,$selecteState=-1){
		global $conn;

		try{
			$query = $conn->prepare('SELECT * FROM state where Country = ?');
			$res = $query->execute(array($country_id));
			$out = $query->fetchAll(PDO::FETCH_ASSOC);
			
			while ($row = array_shift($out)) {
				echo'<option ';
				if($row['State'] == $selecteState){echo "Selected " ;}
				echo 'value="' . $row['State'] .'">' . $row['State']  . '</option>';
			}
		}
		catch(PDOException $e){
			echo $e->getMessage();
