...
function renderPage() {
  ...
  let html = pageMembers.map(m => `
    <div class="bg-white shadow-md rounded-xl p-6 flex flex-col items-center transition hover:scale-105">
      ...
      <div class="flex gap-2 items-center mb-2">
        <span class="inline-block px-2 py-1 bg-blue-50 text-blue-700 rounded">${m.gender}</span>
        ${m.dept ? `<span class="inline-block px-2 py-1 bg-green-50 text-green-700 rounded">${m.dept}</span>` : ""}
      </div>
  ...
  document.getElementById('memberGrid').innerHTML = html || `<div class="col-span-full text-center text-gray-400 py-16">No members found.</div>`;

  // Pagination controls
  let totalPages = Math.ceil(filtered.length / PAGE_SIZE);
  let pagHTML = '';
  if (totalPages > 1) {
    pagHTML += `<button class="px-3 py-1 mx-1 rounded ${page==1?'bg-blue-100 text-blue-400':'bg-blue-600 text-white hover:bg-blue-700'}" ${page==1?'disabled':''} onclick="gotoPage(${page-1})">&laquo; Prev</button>`;
    for (let i=1; i<=totalPages; i++) {
      if (i === 1 || i === totalPages || Math.abs(i-page)<=1) { // show first, last, and nearby
        pagHTML += `<button class="px-3 py-1 mx-1 rounded ${i==page?'bg-blue-700 text-white':'bg-blue-100 text-blue-700 hover:bg-blue-600 hover:text-white'}" onclick="gotoPage(${i})">${i}</button>`;
      } else if (i === page-2 || i === page+2) {
        pagHTML += '<span class="px-2">...</span>';
      }
    }
    pagHTML += `<button class="px-3 py-1 mx-1 rounded ${page==totalPages?'bg-blue-100 text-blue-400':'bg-blue-600 text-white hover:bg-blue-700'}" ${page==totalPages?'disabled':''} onclick="gotoPage(${page+1})">Next &raquo;</button>`;
  }
