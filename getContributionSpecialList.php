...
$query_totalsum = "SELECT tbl_specialcontributions.membersid, (sum(tbl_specialcontributions.contribution)) as contribu 
...
WHERE `Status` = 'Active' AND tbl_specialcontributions.periodid = '". $_GET['periodid']."'";
...
WHERE `Status` = 'Active' AND tbl_specialcontributions.periodid = '". $_GET['periodid']."'  ORDER BY contriId DESC";
$compare = mysqli_query($cov, $query_compare) or die(mysql_error());
...
?>
<?php if ($totalRows_compare > 0) { // Show if recordset empty ?>
<table width="80%" border="1" align="center" cellpadding="4" cellspacing="1">
  <tbody>
    ...
    <?php  $i = 1; do { mysqli_select_db($cov,$database_cov);
...
((sum(tlb_mastertransactionspecial.interestCal))- (sum(tlb_mastertransactionspecial.interestPaid))) as 'interestBalance' 
FROM tlb_mastertransactionspecial WHERE memberid = %s", GetSQLValueString($cov,$row_compare['membersid'], "int"));
$loanBalance = mysqli_query($cov,$query_loanBalance) or die(mysql_error());
...
?>
    ...
    <tr valign="top" align="left">
      ...
      <td align="right" valign="middle" class="greyBgd"><?php echo number_format($row_loanBalance['balance'],2); ?></td>
    ...
    <?php $i = $i + 1; } while ($row_compare = mysqli_fetch_assoc($compare)); ?>
    <tr valign="top" align="left">
