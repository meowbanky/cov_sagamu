...
function GetSQLValueString($conn_vote, $theValue, $theType, $theDefinedValue = "", $theNotDefinedValue = "") 
    {
      // get_magic_quotes_gpc() was removed in PHP 7.4+
      // Magic quotes were deprecated in PHP 5.3 and removed in PHP 5.4, so no stripslashes needed
...
$SearchResult = mysqli_query($cov,$query_SearchResult) or die(mysqli_error($cov));
...
<body>
  ...
  <?php do { ?><tr>
                <td class="dataBox"><input name="btnMRN" type="radio"
                        value="<?php echo $row_SearchResult['memberid']; ?>" onclick="parent.jumptoURL(this)"></td>
                <td class="dataBox"><input name="btnMRN" type="radio"
                        value="<?php echo $row_SearchResult['memberid']; ?>" onclick="parent.jumptoPassport(this)" />
                </td>
    ...
    $dob = $row_SearchResult['DOB'] ?? null;
    if(empty($dob) || $dob == 'NULL' || $dob === null) {
        $dob = date("Y/m/d");
    }
    if(!empty($dob)) {
    ...
    <td class="dataBox"><?php echo $row_SearchResult['MobilePhone']; ?></td>
                <td class="dataBox"><span class="greyBgd"><img src="<?php echo $row_SearchResult['passport']; ?>"
                            alt="passport" name="passport" width="50" height="50" id="passport" /></span></td>
  </tr>
