    ...
    <?php do { ?>
        <?php if ($totalRows_Batch > 0) { // Show if recordset not empty ?>
          <tr valign="top">
            ...
            <td class="greyBgd" valign="middle">&nbsp;</td>
            <td class="greyBgd" valign="middle"><?php mysql_select_db($database_cov, $cov);
$query_suretyInfo = sprintf("SELECT tbl_personalinfo.Lname,tbl_personalinfo.Fname,tbl_personalinfo.Mname FROM tbl_surety
INNER JOIN tbl_personalinfo ON tbl_surety.surety = tbl_personalinfo.memberid WHERE loanid = %s",GetSQLValueString($row_Batch['loanid'], "int"));
...
$s = 1; do { ?>
                                     ...
                                     <?php $s=$s+1;} while ($row_suretyInfo = mysql_fetch_assoc($suretyInfo));
}?></td>
