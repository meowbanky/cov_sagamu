...
$query_compare = "SELECT tbl_personalinfo.patientid, concat(tbl_personalinfo.Lname,' , ',' ',tbl_personalinfo.Fname,' ',tbl_personalinfo.Mname) as namee, ((sum(tlb_mastertransaction.loanAmount) + (sum(tlb_mastertransaction.interest))) - sum(tlb_mastertransaction.loanRepayment)) as loanBalance, tbl_contributions.loan FROM tlb_mastertransaction INNER JOIN tbl_personalinfo ON tbl_personalinfo.patientid = tlb_mastertransaction.memberid INNER JOIN tbl_contributions ON tbl_contributions.membersid = tbl_personalinfo.patientid WHERE `Status` = 'Active'GROUP BY patientid ORDER BY loanBalance desc";
$compare = mysql_query($query_compare, $hms) or die(mysql_error());
$row_compare = mysql_fetch_assoc($compare);
$totalRows_compare = mysql_num_rows($compare);
...
<title>MHWUN, OOUTH -  Loan Contribution Compare</title>
  ...
  <tbody><tr>
    <td>Records Updated successful</td>
                        ...
                        <form action="<?php echo $editFormAction; ?>" method="POST" name="eduEntry" onSubmit="return(validate()); ">
                         <fieldset>
                         <legend class="contentHeader1"> Loan Contribution Compare<a name="top"></a></legend>
                         <table width="80%" align="center" cellpadding="4" cellspacing="0">
                           <tbody>
                             <tr valign="top" align="center">
                               <td width="36%" height="35" valign="middle" class="greyBgd"><strong>Staff No.</strong></td>
                               <td width="64%" colspan="5" valign="middle" class="greyBgd"><strong >Name</strong></td>
                               <td width="64%" valign="middle" class="greyBgd"><strong>Loan Balance</strong></td>
                               <td width="64%" valign="middle" class="greyBgd"><strong>Loan Repayment</strong></td>
                               <td width="64%" valign="middle" class="greyBgd"><strong>Remark</strong></td>
                             </tr>
                             <?php do { ?>
                               <tr valign="top" align="left">
                                 <td class="greyBgd" valign="middle" align="right" height="35"><?php echo $row_compare['patientid']; ?></td>
                                 <td colspan="5" align="left" valign="middle" class="greyBgd"><?php echo $row_compare['namee']; ?></td>
                                 <td align="right" valign="middle" class="greyBgd"><?php echo number_format($row_compare['loanBalance'],2); ?></td>
                                 <td align="right" valign="middle" class="greyBgd"><?php echo number_format($row_compare['loan'],2); ?></td>
                                 <td align="left" valign="middle" class="greyBgd"><?php if($row_compare['loanBalance'] < $row_compare['loan']){echo "<font color=\"#FF0000\"> Reduce Loan Repayment </font>";}else {echo "<font color=\"#00FF00\"> Normal </font>";} ?></td>
                               </tr>
                               <?php } while ($row_compare = mysql_fetch_assoc($compare)); ?>
                           </tbody>
                         </table>
                       </fieldset>
