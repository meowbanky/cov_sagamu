...
require_once('sendmail_online.php');
require_once('sendsms_online.php');
...
function process($memberid, $PeriodID, $pay_method)
{
	...
	mysqli_select_db($cov, $database_cov);
	$query_deductions = "SELECT IFNULL(sum(tbl_contributions.contribution),0) as contri FROM tbl_contributions WHERE membersid = '" . $row_member['memberid'] . "' AND periodid = '" . $_GET['PeriodID'] . "' GROUP BY membersid";
			...
			if ($totalRows_entry > 0) {
				...
				if ($row_deductions['contri'] > 1000) {
					...
					$insertSQLEntryMaster = sprintf(
						"INSERT INTO tlb_mastertransaction (periodid, memberid, entryFee,completed,pay_method) VALUES (%s,%s, %s,%s,%s)",
			...
			if ($totalRows_completed > 0) {
				...
				if ($loanBalance  > 0) {
					...
					if (($contribution == 0)) {
						...
						$insertSQL = sprintf(
							"INSERT INTO tlb_mastertransaction (periodid, memberid, interest, completed,pay_method) VALUES (%s,%s, %s, %s,%s)",
							GetSQLValueString($cov, $_GET["PeriodID"], "int"),
							GetSQLValueString($cov, $row_member['memberid'], "text"),
						...
						$insertSQL = sprintf(
							"INSERT INTO tlb_mastertransaction (periodid, memberid, interest,interestPaid, loanRepayment,savings,completed,pay_method) VALUES (%s,%s, %s,%s, %s,%s,%s,%s)",
							GetSQLValueString($cov, $_GET["PeriodID"], "int"),
							GetSQLValueString($cov, $row_member['memberid'], "text"),
							GetSQLValueString($cov, $currentInterest, "float"),
					...
					$shareSavings = sprintf(
						"INSERT INTO tlb_mastertransaction (periodid, memberid, shares,savings,completed,pay_method) VALUES (%s,%s, %s,%s,%s,%s)",
						GetSQLValueString($cov, $_GET["PeriodID"], "int"),
						GetSQLValueString($cov, $row_member['memberid'], "text"),
				...
				if ($totalRows_Batch > 0) {
					...
					$insertSQL_MasterTransaction = sprintf(
						"INSERT INTO tlb_mastertransaction (periodid, memberid, loanid,loanAmount,pay_method) VALUES (%s, %s, %s, %s,%s)",
						GetSQLValueString($cov, $_GET["PeriodID"], "int"),
						GetSQLValueString($cov, $row_member['memberid'], "text"),
